...
(function (window, document) {
  ...
  _IBSheet.CommonOptions = {
    Cfg: {
      ...
      GroupFormat:
        " <span style='color:red'>{%s}</span> <span style='color:blue'>({%c}건)</span>", // 그룹핑 컬럼명은 빨강색, 건수는 파란색으로 표시
      HeaderSortMode: 1,
      ...
      StorageKeyPrefix: window['sampleName']
        ? window['sampleName']
        : location.href, // 저장 키 prefix 설정
    },
    Def: {
      Header: {
        //헤더 영역 행에 대한 설정
          ...
          OnSave: function (item, data) {
            ...
            if (item) {
              ...
              switch (item.Name) {
                case '컬럼 감추기':
                  if (
                    sheet.id.indexOf('pivotSheet_') == 0 &&
                    item.Owner.Row[col + 'Span'] > 1
                  ) {
                    ...
                    for (var i = 0; i < rowSpan; i++) {
                      ...
                      if (sheet.Cols[mergedCol].Visible)
                        sheet.hideCol(mergedCol, 1, null, 0);
                    }
                    sheet.rerender();
      ...
      onReadMenu: function (evt) {
        if (evt.row.Kind == 'Header') {
          ...
          if (items.Name == '컬럼 표시 여부') {
            ...
            if (items.Items[0].Name == '전체 선택 / 해제')
              items.Items[0].Value = checkCols(sheet);
            return;
    ...
    if (data.length > 0) {
      ...
      var render =
        ...
        (parseInt(version[1]) <= 1 &&
          parseInt(version[2]) == 0 &&
          parseInt(version[3]) < 65)
          ? 1
          : 0;
      ...
      sheet.showMessageTime({ message: '최소 1개의 컬럼은 보여야합니다.' });
  }
  ...
  function setItemBool(items, sheet) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      currentItem.Value = sheet.getAttribute(
        null,
        currentItem.sName,
        'Visible',
      );
  ...
  function mergeRows(arr, headerMerge) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (!nameMap[key] || (nameMap[key] && nameMap[key].sName != item.sName)) {
        ...
        Math.abs(nameMap[key].rowIdx - item.rowIdx) === 1 &&
        headerMerge &&
        headerMerge != 2
      ) {
  ...
  function mergeCol(result) {
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      if (existingItem && existingItem.Items && item.Items)
        existingItem.Items = mergeCol(existingItem.Items.concat(item.Items));
  ...
  window.IB_Preset = {
    ...
    YMD: {
      ...
      Size: 8,
      EmptyValue:
        "<span style='color:#AAA'>년,월,일 순으로 숫자만 입력해 주세요.</span>",
    },
    YM: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 6,
      EmptyValue:
        "<span style='color:#AAA'>년,월 순으로 숫자만 입력해 주세요.</span>",
    },
    MD: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 4,
      EmptyValue:
        "<span style='color:#AAA'>월,일 순으로 숫자만 입력해 주세요.</span>",
    },
    HMS: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 8,
      EmptyValue:
        "<span style='color:#AAA'>시,분,초 순으로 8개 숫자만 입력해 주세요.</span>",
    },
    HM: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 6,
      EmptyValue:
        "<span style='color:#AAA'>시,분 순으로 4개 숫자만 입력해 주세요.</span>",
    },
    YMDHMS: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 14,
      EmptyValue:
        "<span style='color:#AAA'>숫자만 입력(ex:20190514153020)</span>",
    },
    YMDHM: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 12,
      EmptyValue:
        "<span style='color:#AAA'>숫자만 입력(ex:201905141530)</span>",
    },
    MDY: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 8,
      EmptyValue:
        "<span style='color:#AAA'>월,일,년 순으로 숫자만 입력해 주세요.</span>",
    },
    DMY: {
      Type: 'Date',
      Align: 'Center',
      ...
      Size: 8,
      EmptyValue:
        "<span style='color:#AAA'>일,월,년 순으로 숫자만 입력해 주세요.</span>",
    },
    ...
    Popup: {
      ...
      ButtonFormula: function (fr) {
        return fr.Sheet.getCanEdit(fr.Row, fr.Col)
          ? fr.Sheet.Cols[fr.Col].Button
          : '';
      },
      Button:
  ...
  Fn.makeEditEnumList = function (params) {
    ...
    T.ajax({
      ...
      callback: function (res, data) {
        ...
        if (
          resultObj.Enum == null ||
          resultObj.EditEnum == null ||
          resultObj.EnumKeys == null ||
          resultObj.Result === -1
        )
        ...
        T.setAttribute(null, params.colName, 'OnChange', function (e) {
          ...
          if (
            e.sheet.Cols[e.col]['EditEnum'] &&
            e.sheet.Cols[e.col].EditEnum.indexOf(v) > 0
          ) {
  ...
  _IBSheet.v7.convertAcceptKeys = function (objColumn, str) {
    ...
    for (var i = 0; i < acceptKeyArr.length; i++) {
      switch (acceptKeyArr[i]) {
        ...
        default:
          if (
            acceptKeyArr[i].substring(0, 1) == '[' &&
            acceptKeyArr[i].substring(acceptKeyArr[i].length - 1) == ']'
          ) {
            var otherKeys = acceptKeyArr[i].substring(
              1,
              acceptKeyArr[i].length - 1,
            );
...
*/
  _IBSheet.v7.convertDateFormat = function (obj) {
    ...
    for (var i = 0; i < cols.length; i++) {
      ...
      if (obj.sheet.Cols[colName].Type === 'Date') {
        ...
        var format = obj.sheet.Cols[colName].DataFormat
          ? obj.sheet.Cols[colName].DataFormat
          : obj.sheet.Cols[colName].EditFormat
            ? obj.sheet.Cols[colName].EditFormat
            : obj.sheet.Cols[colName].Format.replace(/([^A-Za-z])+/g, '');
        changeCol[colName] = {
          format: format,
  ...
  _IBSheet.v7.convertTreeData = function (data7) {
    ...
    var P = null,
      ...
      c_level = 0,
      p_level = 0;
    targetArr = targetArr.reduce(function (
      accum,
      currentVal,
      curretIndex,
      array,
      ...
      cloneObj['Level'] = cloneObj['level']
        ? cloneObj['level']
        : cloneObj['Level'];
      cloneObj['HaveChild'] = cloneObj['haveChild']
        ? cloneObj['haveChild']
        : cloneObj['HaveChild'];
  ...
  _IBSheet.v7.IBS_Calendar = function (id, format) {
    ...
    function calPickCallBack(v) {
      document.getElementById(id).value = IBSheet.dateToString(
        parseInt(v),
        format,
      );
    }
    var calObj = IBSheet.showCalendar(
      opt,
      {
        Tag: id,
      },
...
-------------------------------------------------------------------------------*/
  _IBSheet.v7.IBS_CopyForm2Sheet = function (param) {
    ...
    if (
      param.form == null ||
      typeof param.form !== 'object' ||
      param.form.tagName !== 'FORM'
    ) {
    ...
    if (typeof row === 'undefined') {
      _IBSheet.v7.IBS_ShowErrMsg(
        'row 인자가 없고, 선택된 행이 존재하지 않습니다.',
      );
      return;
    }
...
-----------------------------------------------------------------------------*/
  ...
  _IBSheet.v7.IBS_FormQueryString = function (form, checkRequired, encoding) {
    if (typeof form === 'string')
      form = document.getElementById(form) || document[form];
    if (typeof form !== 'object' || form.tagName !== 'FORM') {
      _IBSheet.v7.IBS_ShowErrMsg(
        'FormQueryString 함수의 인자는 FORM 태그가 아닙니다.',
      );
      return;
    }
    //default setting
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (checkRequired) {
        ...
        if (
          _IBSheet.v7.IBS_RequiredChk(form.elements[i]) &&
          prev_j != j &&
          value[prev_j] == ''
        ) {
          if (
            form.elements[i].getAttribute('required') == null ||
            form.elements[i].getAttribute('required') == ''
          ) {
            alert(
              '"' +
                _IBSheet.v7.IBS_GetLabel(form.elements[i]) +
                '" 은 필수 입력 항목 입니다.',
            );
          } else {
            alert(
  ...
  _IBSheet.v7.IBS_FormToJson = function (form, checkRequired, encoding) {
    ...
    if (typeof form !== 'object' || form.tagName !== 'FORM') {
      _IBSheet.v7.IBS_ShowErrMsg(
        'FormToJson 함수의 인자는 FORM 태그가 아닙니다.',
      );
      return;
    }
    //default setting
  ...
  _IBSheet.v7.IBS_FromToCalendar = function (id, format) {
    ...
    var oppoValueTimeStamp =
      oppoValue != '' ? IBSheet.stringToDate(oppoValue, format) : null;
    var d =
      ele.value != ''
        ? new Date(ele.value)
        : oppoValue != ''
    ...
    var opt = {
      ...
      OnGetCalendarDate: function (d, dt, cls, r) {
        ...
        if (isFrom) {
          if (d >= targetValueTimeStamp && d <= oppoValueTimeStamp)
            return "<span style='color:orange'>" + dt + '</span>';
        } else {
          if (d <= targetValueTimeStamp && d >= oppoValueTimeStamp)
            return "<span style='color:orange'>" + dt + '</span>';
        }
    ...
    var fromtoCal = IBSheet.showCalendar(
      opt,
      {
        Tag: id,
      },
