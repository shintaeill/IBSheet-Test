...
IBSMSG.Ko = {
  Lang: {
    Alert: {
      ...
      DefaultSheet: '시트',
      NewSheet: '시트%1',
      LoadSheetErrorMissingModule:
        'IBSheet의 %1 모듈을 찾을 수 없어 데이터를 로딩할 수 없습니다.',
      StripHtml:
        '변경시 텍스트 서식이 제거됩니다. 변경사항을 저장하시겠습니까?',
      NoRow: '%1가 없습니다.',
      ...
      InvalidExcelFileExtension:
        '허용되지 않는 파일 형식입니다. 진행을 취소합니다.',
      InvalidTextFileExtension:
        '허용되지 않는 파일 형식입니다. 진행을 취소합니다.',
      LoadExcelError: '엑셀 파일을 읽는 도중 예외가 발생하였습니다',
      ExceedFileSize: '최대 파일 크기를 초과 하였습니다. [최대 크기 : %d]',
      AlertError: 'IBSheet 통신 에러 \r\n\r\nURL: ',
      ResultErrEmptyURL:
        "<span style='color: #ed2f5f; font-weight:bold;'>Url의 값이 없습니다.</span>",
      ResultErrBadURL:
        "<span style='color: #ed2f5f; font-weight:bold;'>Url의 값이 잘못됐습니다.</span>",
      ResultErrNotFound:
        "<span style='color: #ed2f5f; font-weight:bold;'>Url의 주소를 찾을 수 없습니다.</span>",
      ResultErrBadDocumentFormat:
        "<span style='color: #ed2f5f; font-weight:bold;'>문서 형식이 잘못됐습니다.</span>",
      ResultErrEmptyResponse:
        "<span style='color: #ed2f5f; font-weight:bold;'>Url에서 응답이 없습니다.</span>",
      ResultErrRequestTimeout:
        "<span style='color: #ed2f5f; font-weight:bold;'>연결시간이 초과됐습니다.</span>",
      ResultErrBadDataFormat:
        "<span style='color: #ed2f5f; font-weight:bold;'>데이터 형식이 잘못됐습니다.</span>",
      ResultErrUnknown:
        "<span style='color: #ed2f5f; font-weight:bold;'>알 수 없는 오류</span>",
      ApiError: "'%1' 함수에서 오류가 발생하였습니다.",
      RemoveNotWorking: '%1에서는 %2 행을 삭제할 수 없습니다.',
      EditNotWorking: '셀을 편집 할 수 없습니다.',
      ExportDataErrMsg:
        'IBSheet의 JSZip 모듈을 찾을 수 없어 데이터를 다운로드할 수 없습니다.',
      ImportDataErrMsg:
        'IBSheet의 JSZip 모듈을 찾을 수 없어 데이터를 업로드할 수 없습니다.',
      NoXlsxFullMinJS: 'xls 파일 형식은 지원하지 않습니다.',
      CookieNotFound:
        '쿠키를 찾을 수 없습니다. 브라우저에서 쿠키가 허용되어 있는지 확인 바랍니다.',
      TabNameEmpty: '입력한 시트의 이름이 잘못되었습니다.',
      EmptyInfoErr: '시트 정보를 입력해 주세요.',
      InputInfoErr: '입력한 시트 정보가 올바르지 않습니다.',
      NoFilterDataForPivot:
        '피벗 시트를 만들기 위한 필터 데이터가 한 건도 존재하지 않습니다.',
      NoExcelPlugin:
        'XlsImportMode를 사용하시려면 엑셀 플러그인을 포함해주셔야 됩니다.',
    },
    Text: {
      ...
      UpdateCfg: '설정처리중..',
      StartErr:
        "<b style='color:#ff6600;'>시트 표시중 오류가 발생하였습니다.</b>",
      Calculate: '계산중..',
      ...
      Items: '아이템 %d - %d',
      Print:
        '<h2><center>인쇄를 준비하는 동안 잠시 기다려 주십시오.</center></h2>',
      SearchMethodList:
        '|Automatic|Search&#160;like&#160;Google|Search&#160;by&#160;expression',
      ...
      ExportFinished:
        '<center><b>파일이 생성되었습니다.</b><br/><br/>버튼을 클릭하십시오<br/></center><br/>',
      ...
      CopySlow:
        '복사작업이 너무 오래 걸렸습니다.(%d ms), Ctrl+C 를 다시 눌러 작업을 완료하십시오.',
      ...
      LoadSheet: '시트 로딩중',
      DiscardSheet:
        '모든 데이터를 삭제하고 <b>%1</b> 파일의 데이터를 로드 하시겠습니까?',
      AddSheet: '현재 데이터에 <b>%1</b> 파일의 데이터를 추가 하시겠습니까?',
      OpenSheet:
        '모든 데이터를 삭제후 <b>%1</b> 파일의 데이터를 로드 하거나 <br> 현재 데이터에 <b>%1</b> 파일의 데이터를 추가 하시겠습니까?',
      LoadSheetError:
        '<b>%1</b>에서 <b>%2</b> 의 오류로 인해 데이터를 로드하지 못했습니다.',
      ...
      OnlyXlsx:
        '<b>%1</b> 을 (를) 열 수 없습니다. <b> xlsx </b> 파일 만 지원됩니다.',
      ...
      FormulaError11:
        '<b>%1</b> 수식에서 구현되지 않은 Formula 또는 셀 범위 호출 오류',
      MoveRows: '%d개 행이동',
      NoSave: '저장할 내역이 없습니다.',
      ...
      ConfirmSaveDataRows:
        '<div><div>추가: %1건</div><div>수정: %2건</div><div>삭제: %3건</div> 저장하시겠습니까?</div>',
      ConfirmSaveDataRows1:
        '<div><div>추가: %1건</div><div>수정: %2건</div> 저장하시겠습니까?</div>',
      ConfirmSaveDataRows2:
        '<div><div>전체: %1건</div> 저장하시겠습니까?</div>',
      ConfirmSaveDataRows3:
        '<div><div>추가: %1건</div><div>수정: %2건</div><div>삭제: %3건</div><div>이동: %4건</div> 저장하시겠습니까?</div>',
      ...
      StatusLabelChange:
        '$1 열 $2 번째 행 셀의 값이 $3 에서 $4 (으)로 수정되었습니다.',
      ...
      StatusLabelExcel: '엑셀 ',
      StatusLabelFileUpload:
        '$1 열 $2 번째 행 셀의 파일이 $3 (으)로 업로드되었습니다.',
    ...
    MenuCell: {
      ...
      CopySelectedEmpty:
        '%1개 선택된 행(트리)들을 서식만 복사해 대상 위로 추가',
      CopySelectedEmptyBelow:
        '%1개 선택된 행(트리)들을 서식만 복사해 대상 아래로 추가',
      CopySelectedChild: '%1개 선택된 행들을 대상의 자식 행으로 복사 ',
      CopySelectedTreeChild:
        '%1개 선택된 행(트리)들을 대상의 첫 번째 자식 행으로 복사 ',
      CopySelectedEmptyChild:
        '%1개 선택된 행들을 서식만 복사해 대상의 첫 번째 자식 행으로 추가',
      CopySelectedChildEnd:
        '%1개 선택된 행들을 대상의 마지막 자식 행으로 복사 ',
      CopySelectedTreeChildEnd:
        '%1개 선택된 행(트리)들을 서식만 복사해 대상의 마지막 자식 행으로 추가',
      CopySelectedEmptyChildEnd:
        '%1개 선택된 행들을 서식만 복사해 대상의 마지막 자식 행으로 추가',
      CopySelectedEnd:
        '%1개 선택된 행들을 시트 또는 보이는 페이지의 마지막에 복사',
      CopySelectedEndPage: '%1개 선택된 행들을 페이지의 마지막에 복사',
      CopySelectedEndSheet: '%1개 선택된 행들을 시트의 마지막에 복사',
      CopySelectedTreeEnd:
        '%1개 선택된 행(트리)들을 시트 또는 보이는 페이지의 마지막에 복사',
      CopySelectedTreeEndPage:
        '%1개 선택된 행(트리)들을 페이지의 마지막에 복사',
      CopySelectedTreeEndSheet: '%1개 선택된 행(트리)들을 시트의 마지막에 복사',
      CopySelectedEmptyEnd:
        '%1개 선택된 행(트리)들을 서식만 복사해 시트 또는 보이는 페이지의 마지막에 추가',
      CopySelectedEmptyEndPage:
        '%1개 선택된 행(트리)들을 서식만 복사해 페이지의 마지막에 추가',
      CopySelectedEmptyEndSheet:
        '%1개 선택된 행(트리)들을 서식만 복사해 시트 마지막에 추가',
      ...
      CopyRowsEmpty:
        '%1 개의 포커스된 행(트리)들을 서식만 복사해 대상 위에 추가',
      CopyRowsEmptyBelow:
        '%1 개의 포커스된 행(트리)들을 서식만 복사해 대상 아래에 추가',
      ...
      GoDownEditAdd:
        '포커스 아래로 이동(편집 가능한 셀 대상). 아래에 행이 없으면 행 추가후 이동',
      GoLeft: '포커스 왼쪽으로 이동',
      ...
      ShowDetail:
        '마스터 시트 행의 세부 사항을 디테일 시트에 표시(마스터-디테일 구조)',
      RefreshDetail:
        '마스터 시트 행의 세부 사항을 디테일 시트에 표시(마스터-디테일 구조)',
      ...
      ClearSelectedColsStyles:
        '선택된 %1개의 열들에 있는 모든 셀 스타일 초기화',
      ...
      ClearSelectedRowsStyles:
        '선택된 %1개의 행들에 있는 모든 셀 스타일 초기화',
      ClearAll: '모든 셀 값 비우기',
      ...
      ChooseBorderColorItems:
        "<span style='background:%2;padding-left:15px;margin-right:5px;border-radius:50px;height:12px;font-size:12px;'></span>%1",
      ...
      NoTextShadowColorSelected:
        '선택된 %1개 셀의 텍스트 그림자에 적용된 색상 삭제',
      ChooseTextShadowColorSelected:
        '선택된 %1개 셀의 텍스트 그림자에 색상 선택',
    ...
    Debug: {
      ...
      FromTagName: ' << 태그이름',
      NoUploadTag:
        'Upload.Tag 값이 "Debug"로 성정되어 있으면 실제로 업로드는 되지 않고 이 화면에 표시만됩니다.',
      FailedReloadBody: '데이터 리로드 실패: ',
      NoFormTag:
        '<form> 태그를 찾을 수 없습니다. 데이터를 서버로 전송할 수 없습니다.',
      ...
      WrongMergeCell:
        '잘못된 셀 병합 속성이 사용되었습니다. 병합은 쉼표로 구분 된 열 이름 목록으로 설정해야합니다.',
      InCell: ', 셀 -',
      NoDefRow: '정의되지 않은 기본 행',
      CannotAddRow: ', 이 시트에 새 행을 추가 할 수 없습니다',
      FailedLayoutUpdate: '레이아웃 업데이트에 실패했습니다!',
      FailedLayoutUpdateClearLimitScroll:
        'LimitScroll을 지우는 레이아웃 업데이트에 실패했습니다!',
      ConflictObject2:
        '[JavaScript] 글로벌 객체의 이름과 충돌! 해당 객체의 이름을 변경하세요! 다른 스크립트와 충돌되는 IBSheet 글로벌 객체 이름: ',
      ConflictObject1:
        '[JavaScript] 글로벌 객체의 이름과 충돌! 해당 객체의 이름을 변경하세요! IBSheet 글로벌 객체 이름: ',
      NoDataSourceInSheet: '시트를 생성할때 데이터 또는 옵션을 설정해주세요:',
      DataSourceList:
        ' Data.Url, Data.Tag, Data.Data, Layout.Url, Layout.Tag, Layout.Data',
      NoMainTagRequired: 'MainTag는 보여지는 IBSheet를 감싸고 있는 태그입니다.',
      MissingHeight:
        '행 높이 정의가 누락되었거나 잘못된 main.css가 로드되었습니까?',
      EmptyStylePrefix: '빈 스타일 prefix ',
      CompatibleTreeDynSpan:
        '트리의 동적 스패닝은 IE8 / 9 엄격 모드와 호환되지 않습니다! IE를 쿼크 모드 또는 IE7 호환 모드로 전환',
      MissingJSZip: 'JSZip 라이브러리를 포함하십시오.',
      LimitCookie:
        '쿠키가 4096 바이트보다 길면 잘못 저장될 수 있습니다. 이 문제를 해결하려면 PersistentCfg를 사용하십시오.',
      ...
      DupFixedRow1:
        "새로운 고정 행 '$1' 정의를 포함하면이 응답으로 새로운 고정 행을 추가 할 수 없습니다.",
      ReloadBodyDataIn: 'reloadBody: ',
      DupFixedRow2:
        "서버 응답에 새로운 고정 행 '$1'정의가 포함되어 있으며 reloadBody를 통해 새로운 고정 행을 추가 할 수 없으며 레이아웃 $2에서만 고정 행을 정의 할 수 없습니다.",
      DupColumn1:
        "새 열 '$1'정의를 포함하면이 응답으로 새 열을 추가 할 수 없습니다.",
      DupColumn2:
        "서버 응답에 새 열 '$1'정의가 포함되어 있습니다. reloadBody를 통해 새 열을 추가 할 수 없으며 레이아웃 $2에서만 열을 정의 할 수 없습니다.",
      DupSpaceRow1:
        '공백 행 정의를 포함하는 경우 Solid 공백 행을 직접 업데이트 할 수 없습니다. 대신 변경 사항을 사용하십시오.',
      DupSpaceRow2:
        '서버 응답에 Space 행 정의가 포함되어 있습니다. reloadBody를 통해 Space 행을 업데이트 할 수 없으며 레이아웃 $1에서만 Space 행을 정의하십시오.',
      DupBodyRow:
        '옵션에서 행 정의를 포함하면 행을 직접 업데이트 할 수 없으며 대신 Changes를 사용',
      FoundDefRow: '행 기본값중',
      ...
      In: '$1',
      Page: 'page',
      ObsoleteFilterTypeInFilterRow:
        '필터 행에서 더 이상 사용되지 않는 속성 FilterType ',
      ObsoleteAttributeRowsHeader:
        '더 이상 사용되지 않는 속성 행 행에서 더 많은 헤더 행을 정의하십시오.',
      ConflictColAttr: ' 행 속성 또는 브라우저 속성 이름과 충돌',
      ConflictCellAttr: ' 행 또는 셀 또는 브라우저 속성 이름과 충돌',
      ConflictSpaceCellsAttr:
        ' Space 행, $1이 행 또는 다른 셀 또는 브라우저 속성 이름과 충돌',
      IncorrectDefaults: '잘못된 Defaults: ',
      IncorrectEnumMenu: '잘못된 EnumMenu: ',
      NotAvailable: 'IBSheet $1 이용 불가',
      ExportModule: '내보내기 모듈',
      NotSupportedDirectSaving:
        '직접 저장이 지원되지 않으며 누락 된 서버 Export.Url/Script',
      InvalidColor: '잘못된 색상: ',
      NoReturnPage: 'No rows returned for page ',
      DeletedPage: ', page is deleted.',
      OfAttr: ' attribute - ',
      WarnRowHeightTooManyRows:
        '너무 많은 행이 기본 행 높이 ($1px)보다 높기 때문에 행 높이 업데이트가 매우 느립니다. 높이 속성으로 행 높이를 설정하십시오. 처음 다섯 행의 높이는 다음과 같습니다. ',
      DupRowId: '중복된 행 id ',
      ...
      SafeMode: 'Safe mode',
      AffectedExternalCSS:
        ', because IBSheet tag ($1) is affected by external CSS styles, you should remove this influence or set ',
      CannotMaxWidthMaxHeight:
        'Cfg.$1 attribute cannot be used in this layout (e.g. floating or absolute parent without width in pixels), you should set fixed $2 for main <div> tag or set Cfg attribute No$3Scroll / Max$3Scroll. Now there is automatically set ',
      AutoSetMaxWidthMaxHeightPixel1: 'You should set $1 in ',
      AutoSetMaxWidthMaxHeightPixel2:
        ' for main <div> tag or set Cfg attribute $1. Now there is automatically set ',
      AutoSetMaxWidthMaxHeight:
        'You should set $2 for main <div> tag or set Cfg attribute $1. Now there is automatically set $2 of the main <div> to ',
      AutoSetNestedSheetVScroll:
        'Nested sheet should have set NoVScroll or MaxVScroll. Now there is automatically set ',
      ...
      CompletedFullTime3:
        ' (현재 시트의 시작부터 다른 시트 처리에 영향을 받는 실제 시간)',
    ...
    Dialog: {
      ...
      Caution: '경고',
      Caution1:
        "오류 건에는 노란색 배경이 입혀집니다. 오류 건이 있으면 데이터 로드가 되지 않으므로 삭제하거나 수정 후 '확인' 버튼을 클릭하세요.",
      Caution2:
        '업로드 다이얼로그는 엑셀파일의 첫행부터 순서대로 각 열에 대입합니다.',
      Caution3: '숫자형 컬럼에는 숫자만 입력 가능합니다.',
      Caution4:
        '드롭다운형식의 컬럼에는 리스트에 있는 데이터만 입력이 가능합니다.',
      Caution5:
        '체크박스 유형의 컬럼에는  1(선택) 혹은 0(선택해제)로 입력이 가능합니다.',
      Caution6:
        '텍스트 파일 업로드시, 파일에서 사용한 구분자와 다른 구분자로 적용할 경우 시트의 데이터가 올바르게 나오지 않을 수 있습니다.',
      Caution7:
        "현재 오류 건(노란색)이 있습니다. 노란색으로 표시된 부분을 삭제 혹은 수정 후 '확인' 버튼을 클릭하세요.",
      ...
      SortWarn1:
        '모든 정렬 조건에는 지정된 열이 있어야 합니다. 선택한 정렬 조건을 확인하고 다시 시도하십시오.',
      SortWarn2:
        '은(는) 값을 기준으로 두 번 이상 정렬되어 있습니다. 중복된 정렬 조건을 삭제하고 다시 시도하십시오.',
